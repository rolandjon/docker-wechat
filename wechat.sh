...
install(){
  ...
  if ! [ -x ~/.local/bin/wechat.sh ]; then
    ...
    curl -L -H "Cache-Control: no-cache" https://raw.githubusercontent.com/rolandjon/docker-wechat/master/wechat.sh \
    ...
    curl -L -H "Cache-Control: no-cache" https://raw.githubusercontent.com/rolandjon/docker-wechat/master/wechat.png \
...
removei(){
  clean
  imgs=$(docker images | awk '$1 ~ /rlandj\/docker-wechat/ {print $3}')
  [[ -n $imgs ]] && docker rmi $imgs
  return 0
}

clean(){
  container_ids=$(docker ps -a | awk  'NR!=1 && $2 ~ /rlandj\/docker-wechat/ {print $1}')
...
startContainer(){
  ...
  docker container run -d ${arg} \
    ...
    -e GID=`id -g` \
    -e UID=`id -u` \
    rlandj/docker-wechat
  return 0
}

start(){
  container_id=$(docker ps -a | grep script_wechat | awk  '$2 ~ /rlandj\/docker-wechat/ {print $1}')
  if [[ -z "$container_id" ]]; then
    startContainer
  else
    container_stat=$(docker ps | grep script_wechat | awk  '$2 ~ /rlandj\/docker-wechat/ {print $1}')
